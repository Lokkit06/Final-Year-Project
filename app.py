...
def price_chart(df: pd.DataFrame, strategy: str, timeframe_code: str = "M5"):
    ...
    if strategy.startswith("SMA"):
        ...
        for col in ma_cols:
            if col in seen:
                continue
            seen.add(col)
            fig.add_trace(go.Scatter(x=df.index, y=df[col], name=col, line=dict(width=2)), row=1, col=1)
    ...
    if strategy.startswith("RSI") and "RSI" in df:
        fig.add_trace(
            go.Scatter(x=df.index, y=df["RSI"], name="RSI", mode="lines", line=dict(color="#7c3aed", width=2)),
            row=2,
            col=1,
        )
        overbought = 70
        ...
        for level, label in [(overbought, f"Overbought ({overbought:g})"), (oversold, f"Oversold ({oversold:g})")]:
            fig.add_trace(
                go.Scatter(
                    ...
                    name=label,
                    mode="lines",
                    line=dict(color="#a1a1aa", width=1),
                    showlegend=False,
                ),
                row=2,
                col=1,
    ...
    if not rsi_shown and "VOL" in df:
        fig.add_trace(go.Bar(x=df.index, y=df["VOL"], name="Volume", marker_color="lightblue"), row=2, col=1)
...
def comparison_section(df: pd.DataFrame, initial_capital: float = 1000, timeframe_code: str = "M5"):
    ...
    with b1:
        ...
        fig_pnl.update_layout(height=350, xaxis_title="Strategy", yaxis_title="PnL ($)")
    ...
    with b2:
        ...
        fig_dd.update_layout(height=350, xaxis_title="Strategy", yaxis_title="Drawdown (%)")
    ...
    with b3:
        ...
        fig_sharpe.update_layout(height=350, xaxis_title="Strategy", yaxis_title="Sharpe Ratio")
    ...
    with b4:
        ...
        fig_wr.update_layout(height=350, xaxis_title="Strategy", yaxis_title="Win Rate (%)")
        st.plotly_chart(fig_wr, use_container_width=True)

    st.markdown("#### Win/Loss Distribution by Strategy")
    pie_cols = st.columns(4)
    for i, strat_name in enumerate(cmp_df.index):
        with pie_cols[i % 4]:
            wins = cmp_df.loc[strat_name, "Wins"]
            losses = cmp_df.loc[strat_name, "Losses"]
            fig_pie = go.Figure(data=[go.Pie(labels=["Wins", "Losses"], values=[wins, losses], hole=0.35)])
            fig_pie.update_layout(height=320, title=str(strat_name))
...
def trade_log(trades: pd.DataFrame, strategy_name: str):
    ...
    show_cols = [
        col
        for col in ["Entry Time", "Signal", "Entry Price", "Exit Price", "P/L ($)", "P/L (%)"]
