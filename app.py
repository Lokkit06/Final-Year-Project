...
def load_data(timeframe_code: str = "M5") -> pd.DataFrame:
    filename_map = {
        "M5": "XAUUSD.sml_M5_5Y.csv",
        "M15": "XAUUSD.sml_M15_5Y.csv",
        "M30": "XAUUSD.sml_M30_5Y.csv",
        "1H": "XAUUSD.sml_H1_5Y.csv",
    }
...
def price_chart(df: pd.DataFrame, strategy: str, timeframe_code: str = "M5", original_data: pd.DataFrame = None):
    ...
    # Use original_data for full date range display, or df if not provided
    chart_data = original_data if original_data is not None and not original_data.empty else df
    
    # Merge indicator columns from strategy data (df) into chart_data so indicators align with full date range
    if original_data is not None and not original_data.empty and not df.empty:
        # Get indicator columns from strategy data
        indicator_cols = [col for col in df.columns if col.startswith(('SMA_', 'MA_', 'MACD', 'RSI', 'BB_', 'MID_', 'UPPER_', 'LOWER_', 'STRUCTURE'))]
        # Also include Signal and Position columns
        indicator_cols.extend(['Signal', 'Position'])
        # Merge these columns into chart_data, aligning by index
        chart_data = chart_data.copy()
        for col in indicator_cols:
            if col in df.columns:
                # Use reindex to align the Series with chart_data index, filling missing values with NaN
                chart_data[col] = df[col].reindex(chart_data.index)
    
    rows = 2
    # File "C:\Users\kongw\Desktop\Streamlit\Web\app.py", line 395, in price_chart
    fig = make_subplots(
        rows=rows,
        cols=1,
        shared_xaxes=True,
    ...
    fig.add_trace(
        go.Candlestick(
            ...
            increasing_line_color="#16a34a",   # blue
            decreasing_line_color="#dc2626",   # darker blue
            increasing_fillcolor="#16a34a",    # light blue fill
            decreasing_fillcolor="#dc2626",    # medium blue fill
        ),
        row=1,
        col=1,
    )
    ...
    if strategy.startswith("SMA"):
        ma_cols = [c for c in chart_data.columns if c.startswith("SMA_") or c.startswith("MA_")]
        seen = set()
        # Use different, distinct colors for MAs
        ma_colors = ["#f97316", "#2563eb", "#8b5cf6"] 
        for i, col in enumerate(ma_cols):
            if col in seen or col not in chart_data.columns:
                continue
            seen.add(col)
            # Check if column has any non-NaN values
            if chart_data[col].notna().any():
    ...
    if strategy.startswith("MACD") and {"MACD", "MACD_Signal"} <= set(chart_data.columns):
        # Use contrasting colors for MACD and Signal lines
        if chart_data["MACD"].notna().any():
            fig.add_trace(
                go.Scatter(
                    x=chart_data.index, 
                    y=chart_data["MACD"], 
        ...
        if chart_data["MACD_Signal"].notna().any():
            fig.add_trace(
                go.Scatter(
                    x=chart_data.index, 
    ...
    rsi_shown = False
    if strategy.startswith("RSI") and "RSI" in chart_data and chart_data["RSI"].notna().any():
            ...
            if col in chart_data.columns and chart_data[col].notna().any():
                fig.add_trace(
                    go.Scatter(
                        ...
                        y=chart_data[col], 
                        name=col, 
                        line=dict(color=bb_colors.get(col)),
                        mode='lines',
                        showlegend=True
                    ), 
                    row=1, 
    ...
    if "Signal" in chart_data:
        buys = chart_data[chart_data["Signal"] > 0]
        sells = chart_data[chart_data["Signal"] < 0]
        if not buys.empty:
    ...
    # Use chart_data for volume if available, otherwise df
    volume_data = chart_data if "VOL" in chart_data else df
    if not rsi_shown and "VOL" in volume_data:
        fig.add_trace(
            go.Bar(
                x=volume_data.index,
                y=volume_data["VOL"],
...
def best_strategy_across_timeframes(initial_capital: float = 1000, start_date=None, end_date=None):
    """Calculate and display the most profitable strategy across all 4 timeframes."""
    ...
    if start_date > end_date:
        return  # Invalid date range, skip calculation
    
    start_dt = pd.Timestamp(start_date)
    end_dt = pd.Timestamp(end_date)
    date_range_months = (end_dt - start_dt).days / 30.44  # Average days per month
    
    if date_range_months < 6:
        st.markdown("<div style='margin-top:40px;'></div>", unsafe_allow_html=True)
        st.error("Please select a date range of at least 6 months for accurate results. The current selection is less than 6 months.")
        st.markdown("<div style='margin-bottom:40px;'></div>", unsafe_allow_html=True)
        return  # Don't display if less than 6 months
    ...
    for tf_code, tf_display in timeframe_map.items():
        df = load_data(tf_code)
        if df.empty:
            continue
    ...
    # Find best strategy by Sharpe Ratio (the single most important metric for ranking)
    # Filter out strategies with invalid or zero Sharpe Ratios
    valid_results = results_df[
        (results_df["Sharpe Ratio"].notna()) & 
        (results_df["Sharpe Ratio"] > 0) &
    ...
    # Sort by Sharpe Ratio (descending) - highest ratio wins
    valid_results = valid_results.sort_values(by="Sharpe Ratio", ascending=False)
    best = valid_results.iloc[0]
    
    # Display the statement
    st.info(
        f"**Most Profitable Strategy Across All Timeframes:** {best['Strategy']} on {best['Timeframe']}"
    )



def comparison_section(df: pd.DataFrame, initial_capital: float = 1000, timeframe_code: str = "M5", start_date=None, end_date=None):
    timeframe_display_map = {
        "M5": "5 Minutes",
        "M15": "15 Minutes",
        "M30": "30 Minutes",
    ...
    # Get timeframe selection - positioned in upper right
    comparison_timeframe_options = ["5 Minutes", "15 Minutes", "30 Minutes", "1 Hour"]
    current_display = timeframe_display_map.get(timeframe_code, "5 Minutes")
    current_index = comparison_timeframe_options.index(current_display) if current_display in comparison_timeframe_options else 0
    
    # Position timeframe selector in upper right (at top for functionality, visually appears above table)
    col_header_left, col_header_right = st.columns([10, 2])
    with col_header_left:
        pass  # Empty space
    with col_header_right:
        comparison_timeframe_display = st.selectbox(
            "Timeframe",
            comparison_timeframe_options,
            index=current_index,
            key="comparison_timeframe_selector",
            label_visibility="collapsed"
        )
    
    comparison_timeframe_code = {
        "5 Minutes": "M5",
        "15 Minutes": "M15",
        "30 Minutes": "M30",
        "1 Hour": "1H",
    }.get(comparison_timeframe_display, "M5")
    
    # Load data for the selected comparison timeframe
    comparison_df = load_data(comparison_timeframe_code)
    if comparison_df.empty:
        st.warning(f"No data available for {comparison_timeframe_display} timeframe.")
        return
    
    # Filter by date range if provided (from sidebar filters) - no max bars limit
    if start_date is not None and end_date is not None:
        ...
        start_dt = pd.Timestamp(start_date)
        end_dt = pd.Timestamp(end_date) + pd.Timedelta(days=1) - pd.Timedelta(seconds=1)
        comparison_df = comparison_df.loc[start_dt:end_dt]
        
        # Validate that date range contains data
        if comparison_df.empty:
            st.warning(f"⚠️ No data available for the selected date range ({start_date} to {end_date}) in {comparison_timeframe_display} timeframe.")
            return
    
    # Calculate strategies with the filtered data (no max bars limit)
    strategies = {
        "SMA 8 / 32": run_strategy_with_split(sma_strategy, comparison_df, DEFAULT_SPLIT_RATIO, 8, 32),
        "SMA 16 / 64": run_strategy_with_split(sma_strategy, comparison_df, DEFAULT_SPLIT_RATIO, 16, 64),
        "SMA 20 / 80": run_strategy_with_split(sma_strategy, comparison_df, DEFAULT_SPLIT_RATIO, 20, 80),
        "MACD 12 / 26 / 9": run_strategy_with_split(macd_strategy, comparison_df, DEFAULT_SPLIT_RATIO, 12, 26, 9),
        "MACD 10 / 30 / 8": run_strategy_with_split(macd_strategy, comparison_df, DEFAULT_SPLIT_RATIO, 10, 30, 8),
        "MACD 20 / 50 / 10": run_strategy_with_split(macd_strategy, comparison_df, DEFAULT_SPLIT_RATIO, 20, 50, 10),
        "RSI 14 / 30 / 70": run_strategy_with_split(rsi_strategy, comparison_df, DEFAULT_SPLIT_RATIO, 14, 30, 70),
        "RSI 9 / 20 / 80": run_strategy_with_split(rsi_strategy, comparison_df, DEFAULT_SPLIT_RATIO, 9, 20, 80),
        "RSI 21 / 40 / 60": run_strategy_with_split(rsi_strategy, comparison_df, DEFAULT_SPLIT_RATIO, 21, 40, 60),
        "BB 20 / 2": run_strategy_with_split(bb_strategy, comparison_df, DEFAULT_SPLIT_RATIO, 20, 2.0),
        "BB 10 / 2": run_strategy_with_split(bb_strategy, comparison_df, DEFAULT_SPLIT_RATIO, 10, 2.0),
        "BB 20 / 1.5": run_strategy_with_split(bb_strategy, comparison_df, DEFAULT_SPLIT_RATIO, 20, 1.5),
    }
    comps = {}
    ...
    for row_idx, (prefix, family_label) in enumerate(strategy_families):
        ...
        # Add extra vertical spacing and divider line before all rows except the first
        if row_idx > 0:
            st.markdown("<div style='margin-top:40px;'></div>", unsafe_allow_html=True)
            st.markdown("---")  # Divider line between strategy families

        st.markdown(f"<h2 style='font-size: 32px; font-weight: bold; margin-bottom: 20px;'>{family_label}</h2>", unsafe_allow_html=True)
    ...
    # Overall Strategy Summary title
    st.markdown("<h2 style='font-size: 32px; font-weight: bold; margin-bottom: 20px;'>Overall Strategy Summary</h2>", unsafe_allow_html=True)
    ...
    # Display most profitable strategy across all timeframes below the table
    best_strategy_across_timeframes(initial_capital, start_date, end_date)
...
def trade_log(trades: pd.DataFrame, strategy_name: str):
    ...
    show = entries[show_cols].reset_index(drop=True)
...
def main():
    ...
    # Variation dropdowns placed directly under Strategy
    if strategy_key == "sma":
        sma_variations = {
            "SMA 16 / 64": (16, 64),
            "SMA 8 / 32": (8, 32),
    ...
    start_dt = pd.Timestamp(start_date)
    end_dt = pd.Timestamp(end_date) + pd.Timedelta(days=1) - pd.Timedelta(seconds=1)
    # Full date-filtered data for all calculations (metrics, comparison, etc.)
    data_full = df.loc[start_dt:end_dt]
    total_selected = len(data_full)
    
    # Validate that date range contains data
    if data_full.empty:
        st.error(f"⚠️ No data available for the selected date range ({start_date} to {end_date}). Please select a different date range.")
        st.info(f"Available data range: {min_date} to {max_date}")
        st.stop()
    
    # Limited data only for chart display
    data_chart = data_full.copy()
    if max_bars < total_selected:
        data_chart = data_chart.iloc[:max_bars]
        actual_end = data_chart.index[-1].date() if not data_chart.empty else end_date
        st.caption(f"Showing the first {max_bars:,} bars from {start_date} to {actual_end} (out of {total_selected:,} total bars in selected range).")

    # Run strategy with the FULL date range data (not limited by max_bars)
    if strategy_key == "sma":
        sdata = run_strategy_with_split(sma_strategy, data_full, DEFAULT_SPLIT_RATIO, fast, slow)
    elif strategy_key == "macd":
        sdata = run_strategy_with_split(macd_strategy, data_full, DEFAULT_SPLIT_RATIO, fast, slow, sig)
    elif strategy_key == "rsi":
        sdata = run_strategy_with_split(rsi_strategy, data_full, DEFAULT_SPLIT_RATIO, period, oversold, overbought)
    else:
        sdata = run_strategy_with_split(bb_strategy, data_full, DEFAULT_SPLIT_RATIO, period, std)
    ...
    with c5:
        # Max Drawdown is negative or zero. Always display in red.
        kpi("Max Drawdown", f"{m['Max Drawdown (%)']:.2f}%", positive=False) 
    with c6:
        kpi("Sharpe Ratio", f"{m['Sharpe Ratio']:.2f}", positive=m["Sharpe Ratio"] >= 1)

    # Use limited data_chart only for chart display
    price_chart(sdata, strat_name, timeframe_code, original_data=data_chart)
    # Use full data_full for comparison section
    comparison_section(data_full, initial_capital, timeframe_code, start_date, end_date)
    st.markdown("---")
