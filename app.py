...
def main():
    ...
    if max_bars < total_selected:
        ...
        actual_end = data.index[-1].date() if not data.empty else end_date
    ...
    # Run strategy with the selected variation
    if strategy_key == "sma":
    ...
    price_chart(sdata, strat_name, timeframe_code, original_data=data)
    comparison_section(data, initial_capital, timeframe_code, start_date, end_date)
    st.markdown("---")
